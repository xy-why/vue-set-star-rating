  ...
  <div>
    <div class="star-rating">
      <div v-for="(star, index) in stars" :key="index" class="star-container"
        @click="clickChange && nowStar($event,index)">
        <svg class="star-svg" :style="[
          ...
          <polygon :points="getStarPoints" style="fill-rule: evenodd" />
          <defs>
            <!--
...
<script>
  export default {
    ...
    methods: {
      nowStar(e, i) {
        let num = Math.round(e.layerX/10);
        if(num>=9.5){
          this.$emit('itemclick', i+1, i,e)
        }else{
          this.$emit('itemclick', Number(i+ "." +num), i,e)
        }
